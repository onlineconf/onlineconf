...
Чтобы добавить новые значения выполните следующие шаги: 
1. Создайте новое значение в `/onlineconf/ephemeral-ip` типа "Текст" или "JSON"
2. Запишите значения вида ip/mask (в случае json - используйте json массив, в случае если вы используете строку, перечислите значения, разделяя символом `,`)
