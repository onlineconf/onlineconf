...
* `header` - аутентификация пользователя осуществляется механизмами reverse proxy, развернутого перед *onlineconf-admin*, который в случае успеха передает дополнительный HTTP заголовок `X-Username` с именем аутентифицированного пользователя (плюс, опционально, подпись `X-Username-Sign`). Подпись - это md5 хеш от конкатенации параметров `X-Username`, `remote_addr` и параметра конфигурации onlineconf-admin `secret`
