...
* `header` - аутентификация выполняется обратным прокси-сервером, развернутым перед onlineconf-admin. Аутентификация этого типа требует параметра конфигурации `behind_reverse_proxy` и одного дополнительного параметра конфигурации `header`, который устанавливает имя заголовка с `username`. Например, если вы используете oauth2-proxy, имя заголовка с именем пользователя — «X-Forwarded-Preferred-Username».
Как это работает. В случае успеха обратный прокси добавляет один обязательный HTTP-заголовок с именем пользователя и один дополнительный HTTP-заголовок с подписью. По умолчанию имя заголовка с именем пользователя — «X-Username». Заголовок, содержащий подпись, использует то же имя, но с постфиксом `-Sign` (по умолчанию `X-Username-Sign`). Подпись рассчитывается как хешированная строка MD5, состоящая из необходимого значения заголовка (по умолчанию `X-Username`), `remote_addr` и секретного параметра конфигурации onlineconf-admin.
