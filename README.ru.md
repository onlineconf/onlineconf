...
`/onlineconf/group` - группировка серверов и сервисов по имени. Имя параметра - имя группы, значение - *glob* (примерно как в `bash`) hostname серверов, входящих в эту группу, или имен сервисов с префиксом `service:` (например: `service:gopher/alpha`). Синтаксис глобов описан здесь: https://github.com/gobwas/glob. Например: `{alpha*.{example.com,i},service:gopher/alpha}` - включает сервера с hostname `alpha*.example.com`, `alpha*.i` (где `*` - любые символы кроме `.`) и сервис `gopher/alpha`.
