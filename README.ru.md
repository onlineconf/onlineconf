...
* `header` - аутентификация пользователя осуществляется механизмами reverse proxy, развернутого перед *onlineconf-admin*, который в случае успеха передает дополнительный HTTP заголовок, например `X-Forwarded-Preferred-Username` (как oauth2-proxy) с именем аутентифицированного пользователя, плюс, опционально, подпись `X-Forwarded-Preferred-Username-Sign` (md5 хеш от конкатенации параметров `X-Username`, `remote_addr` и параметра конфигурации onlineconf-admin `secret`). Имя заголовка конфигурируется в зависимости от потребности. Этот тип авторизации требует обязательное включение настройки behind_reverse_proxy.
