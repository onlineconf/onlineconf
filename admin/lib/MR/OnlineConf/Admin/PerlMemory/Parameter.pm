...
has dirty => (
    ...
    isa => 'Bool',
    default => 0,
    trigger => sub { MR::OnlineConf::Admin::PerlMemory::Transaction->add_to_dirty($_[0]) if $_[1] },
);
...
sub resolve_case {
    my ($self, $datacenter, $groups, $host) = @_;
    $self->clear();
    $self->_resolve_case($self->data, $datacenter, $groups, $host);
    return;
}

sub _resolve_case {
    my ($self, $data, $datacenter, $groups, $host) = @_;
    $data = JSON::from_json($data);
    my %groups = map { $_ => 1 } @$groups;
    foreach my $case ($self->_sort_case($data, $groups)) {
        if (exists $case->{server}) {
            if (Text::Glob::match_glob($case->{server}, $host)) {
