...
sub reloadLocal {
    ...
    while (my ($n,$f) = each %$files){
        ...
        $linked{$_}{$data->{Name}} = 1 foreach grep { $_ ne $data->{Name} } map $_->{TargetModule}, @{$data->{SymLink}};
    }
    $self->{local}{$_}{LinkedModules} = [ keys %{$linked{$_}} ] foreach keys %linked;
    return 1;
}
    ...
    foreach my $m (@$modules){
        unless ($self->{local}{$m->{Name}} && $self->{local}{$m->{Name}}{Version} == $m->{Version}){
            if($m->{Version} > 0) {
                ...
                my @linked = @{$self->{local}{$m->{Name}}{LinkedModules}||[]};
                while (my $linked = shift @linked) {
                    unless ($to_update{$linked}) {
                        $to_update{$linked} = 1;
                        push @linked, @{$self->{local}{$linked}{LinkedModules}||[]};
                    }
                }
            }
        }
...
sub update {
    ...
    foreach my $k (keys %$data){
        ...
        my (%data, @symlink);
        foreach my $param (values %{$data->{$k}}) {
            if (($param->{Flags} & MY_CONFIG_SYMLINK_FLAG) && $param->{Value} =~ /^(\S+?):(\S+)$/) {
                push @symlink, { LinkName => $param->{Key}, TargetModule => $1, TargetName => $2 };
                my $i = 20;
                my $target = $param;
                while ($target && ($target->{Flags} & MY_CONFIG_SYMLINK_FLAG) && $target->{Value} =~ /^(\S+?):(\S+)$/) {
                    if (exists $data->{$ids{$1}->{ID}}) {
                        $target = $data->{$ids{$1}->{ID}}->{$2};
                    } elsif (exists $cache{$ids{$1}->{ID}}) {
                    ...
                    unless ($i--) {
                        $self->_say(-1, "recursive symlink $param->{Key}\n");
                        $target = undef;
                    }
                }
                $data{$param->{Key}} = $target->{Value} if $target;
            } else {
...
sub updateComposite {
    my ($self) = @_;
    my %data;
    my $files = $self->localList() or return;
    while (my ($n,$f) = each %$files){
        my $data = $self->readFile($n,md5_check=>0);
    ...
    my $str = $self->dump({ Name => 'ALL', Version => 0, Data => \%data });
    unless (open F , '>'.$self->LOCAL_CFG_PATH().'ALL.conf_tmp'){
        $self->_say(-1,"cant open ".$self->LOCAL_CFG_PATH()."ALL.conf_tmp for writing: $!\n");
        return;
    }
    print F $str;
    close F;
    unless (rename $self->LOCAL_CFG_PATH().'ALL.conf_tmp' , $self->LOCAL_CFG_PATH().'ALL.conf'){
        $self->_say(-1,"cant rename ".$self->LOCAL_CFG_PATH()."ALL.conf_tmp to to .conf: $!");
    }
    return;
}
...
sub _get_overload {
    ...
    my $data = $self->{storage}->getAll($self->{storage}->getModuleIDByName(MY_CONFIG_OVERLOAD_MODULE_NAME),json2perl=>0);
...
sub _dump_plain {
    ...
    foreach my $symlink (@{$data->{SymLink}}) {
        $s.="#@ $symlink->{LinkName} $symlink->{TargetModule}:$symlink->{TargetName}\n";
    }
